...
function ex_constquadpartials()
    ...
    # Linear strike-slip
    slipconst[1:2:end] = 0.001 * xcenters
    slipquad[1:2:end] = 0.001 * xnodes
    ...
    uquadnear, σquadnear = quaduσ(slip2uσ, xnodes, 1 .+ zeros(length(xnodes)), els, srcidx, repeat(0.001 .* xcenters, 1, 3), zeros(length(srcidx), 3), μ, ν)
 
    # display(repeat(0.001 .* xcenters, 1, 3))
    # repeat(0.001 .* xcenters, 1, 3)
