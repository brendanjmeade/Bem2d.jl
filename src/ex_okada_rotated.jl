...
function ex_okada()
    ...
    # x1, y1, x2, y2 = Bem2d.discretizedline(0, -0.5, 0, 0.5, nfault)
    x1, y1, x2, y2 = Bem2d.discretizedline(-sqrt(2)/4, -sqrt(2)/4, sqrt(2)/4, sqrt(2)/4, nfault)
    ...
    # Constant slip fault
    slipdiag = sqrt(2)/2
    dispbemss, stressbemss = Bem2d.constdispstress(slip2dispstress, x, y, els, idx["fault"], slipdiag, slipdiag, mu, nu)
    ...
    for i in 1:length(x)
        ...
        # Tensile-slip
        _, u, gradu = ow.dc3dwrapper((2.0/3.0), [0.0, x[i], y[i]-deep], 0.0 + deep, 45.0, [-1000000, 1000000], [-0.5, 0.5], [0.0, 0.0, slipdiag])
        strain = @. 0.5 * (gradu' + gradu)
        stress = mu*LinearAlgebra.I(3)*tr(strain) + 2.0*mu*strain
        dispokadats[i, 1] = u[2]
        dispokadats[i, 2] = u[3]
