...
function plotvelocities()
    ...
    PyPlot.contourf(plotme, 10, cmap = rycroftcmap())
    PyPlot.colorbar(fraction = 0.020, pad = 0.05, extend = "both", label = L"$\log_{10}v$ (m/s)")
    PyPlot.contour(plotme, 10, linewidths = 0.5, linestyles = "solid", colors = "k")
    ...
    PyPlot.contourf(plotme, 20, cmap = get_cmap("PRGn"), vmin=-1e-7, vmax=1e-7)
    PyPlot.colorbar(fraction = 0.020, pad = 0.05, extend = "both", label = L"$block motion - slip$ (m/s)")
    PyPlot.contour(plotme, [0], linewidths = 0.5, linestyles = "solid", colors = "k")
    PyPlot.xlabel("time step", fontsize=fontsize)
    PyPlot.ylabel("element index", fontsize=fontsize)
