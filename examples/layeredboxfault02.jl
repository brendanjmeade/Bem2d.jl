...
"""
    mirrorels!(els, idx, label)

Create a copy of the elements with indices idx adding them to the
else structure and giving them the new name label. But...the normals
node ordering are reversed.
"""
function mirrorels!(els, idx, label)
    addelsez!(els, els.x2[idx], els.y2[idx], els.x1[idx], els.y1[idx], label)
end
...
function layeredboxfault()
    ...
    # Try with cloning and flipping normals as well as rotmat?
    mirrorels!(elslayer, findall(x->x == "B1", elslayer.name), "T2") # Create shared common boundary
